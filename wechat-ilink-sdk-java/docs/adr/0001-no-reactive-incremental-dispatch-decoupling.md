...
- dispatch executor 队列当前**无界**：持续过载时可能堆积（消费方连续长轮询节拍下风险低）——原计划随 P2 处理；P2 撤销后此债**明确搁置**（若日后过载成真，方案是有界队列 + CallerRunsPolicy，天然背压且不引入依赖）。
...
## 后续修复（2026-07，漏洞重分析落地）

复核本 ADR 落地效果时发现三处同族缺陷，按同样的增量原则修复（零对外 API 破坏，首批 JUnit 5 特征化测试随附）：

1. **登录轮询热循环 + 取消失效**：`LoginService` 状态轮询原先无间隔连发请求，且 `CompletableFuture.cancel(true)` 并不会中断 `supplyAsync` 任务、循环检查的中断位永远不翻转（取消后僵尸轮询跑满 loginTimeoutMs）。修复：新增 `loginPollIntervalMs`（默认 1500ms）+ 每次轮询持有显式 `AtomicBoolean` 取消标志。
2. **登录/心跳监听器隔离补齐**：`executeLogin.whenComplete` 与 `HeartbeatService` tick 原先裸调监听器——一个抛异常会中断其余监听器、阻断 `heartbeatService.start()`、把 success 误报成 failure。修复：逐监听器 try/catch（与 P0 的 `dispatchMessages`/P3 的 `fireOnDisconnect` 同款），心跳启动移到监听器通知之前。
3. **看门狗盲区**：`checkLiveness` 原先 `lastPollSuccessAt==0` 直接返回——消费方登录后从未跑接收循环（恰是本 ADR 之后最该报警的契约违约）时看门狗永远沉默。修复：心跳启动时武装基线为当前时间。

单独决策、本轮不动：HTTP 4xx 快速失败（见 P2 备注）；发送类 POST 重试的重复发送风险（协议无幂等键，已在 README 声明）；`close()` 语义收紧（同 P3 不翻 `isLoggedIn` 的顾虑）。
