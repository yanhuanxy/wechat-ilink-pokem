...
public class StealRecordRepository {
    ...
    /** 记一次偷取。PK 冲突（同一贼同一地同一轮已偷）返回 false，调用方据此判定"重复偷"。 */
    public boolean record(String victimId, int plotIndex, String plantedAt, String thiefId, int amount) {
        String sql = "INSERT OR IGNORE INTO steal_record " +
                "(victim_id, plot_index, planted_at, thief_id, amount, stolen_at) VALUES (?, ?, ?, ?, ?, ?)";
